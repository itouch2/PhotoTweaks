             ...
             maxRotationAngle:(CGFloat)maxRotationAngle
{
  if (self = [super init]) {
    ...
    self.btnOriginX = self.frame.size.width * 0.10;
    self.btnMargin = self.frame.size.width * 0.07;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  } else if (CGRectContainsPoint(self.aspectBtn.frame, point)) {
    return self.aspectBtn;
  } else if (CGRectContainsPoint(self.rotationBtn.frame, point)) {
    return self.rotationBtn;
...
- (void)rotateImage:(id)sender
{
  ...
  [self.slider sendAction:@selector(sliderValueChanged:) to:nil forEvent:nil];
  [self.slider sendAction:@selector(sliderTouchEnded:) to:nil forEvent:nil];
